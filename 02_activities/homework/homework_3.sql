...
SELECT
	vendor_id,
	COUNT(*) AS number_times_vendor_rented_booth
...
-- Create a temporary table with data from 'vendor' table
...
-- Create a second temporary table with the same structure as the previous one
CREATE TEMP TABLE new_new_vendor_inventory AS
SELECT *
FROM new_vendor_inventory;

-- Insert specific values into the new_new_vendor_inventory table
INSERT INTO new_new_vendor_inventory (col1, col2, col3, col4, col5)
...
FROM
  customer_purchases
WHERE
  STRFTIME('%m', purchase_date) = '04'  -- Filter for April
  AND STRFTIME('%Y', purchase_date) = '2022'  -- Filter for 2022
GROUP BY
  customer_id;
